...
**Status**: Phase 1 complete. P1.1–P1.10 complete. Phase 2 complete through P2.4 (`28702b95`) with style guard green. Phase 3 complete through P3.6; P3.5 extension recipes authored — 16 of 18 done (`ai.md` + 15 eligible recipes in `cf4394a6` + `437c3a6c`); `scenes.md` and `resources.md` remain gated on Phase 4. Phase 4 scripts split complete (P4.1–P4.4): all scripts in `scripts/core/` or `scripts/demo/`, boundary enforcement green. P4.5–P4.9 (resources/scenes/assets split) not yet started. `AGENTS.md` is now a 58-line routing index and `docs/guides/DEV_PITFALLS.md` has been deleted after redistribution. `ai-system-overview.md` updated to remove stale GOAP/HTN content.
**Next Task**: P4.5 Resources & Scenes Audit — produce `docs/guides/cleanup_v8/resources_scenes_audit.md` enumerating every resource and scene file requiring move, with import/reference path impact analysis and commit sequence proposal. Then proceed through P4.6 (resources core move), P4.7 (scenes split), P4.8 (assets demo move), P4.9 (enforcement).
...
3. **Phase 3 — Docs Split.** Break `AGENTS.md` + `DEV_PITFALLS.md` into focused, topic-scoped files. `AGENTS.md` becomes a routing index (<150 lines). Phase 3 doc creation requires per-recipe user authorization; the V8 commit is not a blanket yes. Pre-execution decisions (2026-04-23): ADR numbering preserves V7.2 F5 at `0001`, pre-existing ADRs shift to `0002..0005`, V8 new ADRs land at `0006..0010`; the existing service-locator ADR (post-rename `0005`) gets an in-place amendment rather than a new `0007` file; P3.5 Phase 3 deliverable is the recipe framework only (dir + `README.md` + template + structure test), individual recipes at phase tails.
4. **Phase 4 — Template vs Demo Split.** Reorganize `scripts/`, `resources/`, `scenes/`, and `assets/` into `core/` (template) and `demo/` (examples). P4.1–P4.4 (scripts) complete. P4.5–P4.9 (resources/scenes/assets) next: audit → resources core move → scenes split → assets demo move → enforcement. Consumers can delete `demo/` without breaking the template.
...
- **Phase 1**: COMPLETE.
  ...
  - **P1.6b** complete:
    ...
    - `(GREEN) P1.6b wire planner debug snapshot into AI brain debug panel` (`64de76f6`)
    - `(GREEN) P1.6b style-enforcement follow-through for planner boundaries/LOC caps` (`7364b41f`)
  - **P1.7** complete:
    - `(RED) P1.7 add C_AIBrainComponent BT contract test` (`6385e68d`)
  ...
  - **Verification state**:
    ...
    - New P1.6b planner debug snapshot test is green (`tests/unit/ecs/components/test_c_ai_brain_component.gd::test_get_debug_snapshot_prefers_latest_planner_debug_from_task_state`).
    - New P1.6b planner debug panel rendering test is green (`tests/unit/debug/test_debug_ai_brain_panel.gd::test_panel_row_text_renders_planner_plan_when_present`).
    - New P1.6b planner boundary/LOC style checks are green (`tests/unit/style/test_style_enforcement.gd::test_bt_general_utils_do_not_reference_ai_specific_types`, `::test_bt_planner_scripts_stay_within_loc_caps`).
    - New P1.7 brain-component BT contract tests are green (`tests/unit/ecs/components/test_c_ai_brain_component_bt.gd`).
    ...
    - P1.8 targeted checks are green in branch history:
    ...
    - P1.9 Commit 1 RED check failed for expected reason:
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` failed (`cfg_wolf_brain_bt.tres` missing).
    - P1.9 Commit 2 GREEN checks:
    ...
    - P1.9 Commit 3 RED+GREEN checks:
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_deer_brain_bt.gd` RED check fails for expected reason (`cfg_deer_brain_bt.tres` missing), then passes after GREEN migration (`3/3`).
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` recheck passes (`2/2`).
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` recheck passes (`3/3`).
    - P1.9 Commit 4 RED+GREEN checks:
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_rabbit_brain_bt.gd` RED check fails for expected reason (`cfg_rabbit_brain_bt.tres` missing), then passes after GREEN migration (`3/3`).
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_deer_brain_bt.gd` recheck passes (`3/3`).
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` recheck passes (`2/2`).
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` recheck passes (`3/3`).
    - P1.9 Commit 5 RED+GREEN checks:
      - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_patrol_drone_brain_bt.gd` RED check fails for expected reason (`cfg_patrol_drone_brain_bt.tres` missing), then passes after GREEN migration (`3/3`).
...
- **Phase 4**: IN PROGRESS. P4.1–P4.4 (scripts split) complete — all scripts in `scripts/core/` or `scripts/demo/`, core-never-imports-demo boundary enforcement green (87/87). P4.5–P4.9 (resources/scenes/assets split) not yet started. P4.5 is the next task: audit all `.tres` and `.tscn` files against `template_vs_demo.md` classification, produce detailed migration checklist with commit sequence proposal. `resources/demo/` already contains 107 demo files (AI brains, demo audio, demo base_settings/gameplay, demo color gradings, demo interactions, demo lighting, demo scene_registry, demo spawn_metadata). `resources/core/` is empty — ~170 core `.tres` files remain in flat `resources/` subdirs. `scenes/` is entirely unsplit. `assets/` is entirely unsplit.
...
- P1.6b Commit 11 GREEN landed in `64de76f6` (`scripts/ecs/components/c_ai_brain_component.gd`, `scripts/demo/debug/debug_ai_brain_panel.gd`) with planner `last_plan`/`last_plan_cost` snapshot propagation and debug row rendering.
- P1.6b Commit 11 targeted checks:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ecs/components/test_c_ai_brain_component.gd` passes (`16/16`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/debug/test_debug_ai_brain_panel.gd` passes (`4/4`).
- P1.6b Commit 12 GREEN (`7364b41f`) check: `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` passes (`62/62`) after adding BT utils AI-boundary checks + planner LOC caps.
- P1.6b Commit 12 targeted checks:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/bt/test_rs_bt_planner.gd` passes (`9/9`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/bt/test_u_bt_planner_search.gd` passes (`9/9`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ecs/components/test_c_ai_brain_component.gd` passes (`16/16`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/debug/test_debug_ai_brain_panel.gd` passes (`4/4`).
- Current planner check: `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/bt/test_rs_bt_planner.gd` passes (`9/9`).
- Current style check: `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` passes (`62/62`).
- Current full suite check: `tools/run_gut_suite.sh` completed with no failing tests (`4553` passing / `8` pending).
- P1.7 Commit 1 RED (`6385e68d`) check: `tools/run_gut_suite.sh -gtest=res://tests/unit/ecs/components/test_c_ai_brain_component_bt.gd` fails for expected BT-contract reasons (`bt_state_bag` absent, legacy fields still present).
- P1.7 Commit 1 style check: `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` passes (`62/62`).
- P1.7 Commit 2 GREEN (`fbcaccd9`) checks:
...
- P1.7 Commit 3 RED (`54425b93`) check: `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/resources/test_rs_ai_brain_settings_bt.gd` fails for expected reasons (`root` missing, legacy `goals` still present, no migration `push_error`).
- P1.7 Commit 3 style check: `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` passes (`62/62`).
- P1.7 Commit 4 GREEN (`bf2a734e`) checks:
...
- P1.9 Commit 1 RED check (new):
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` fails for expected reason (`cfg_wolf_brain_bt.tres` missing).
- P1.9 Commit 2 GREEN checks (new):
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` passes (`2/2`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` passes (`3/3`).
- Current style check after P1.9 Commit 2:
...
- P1.9 Commit 3 RED check:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_deer_brain_bt.gd` fails for expected reason (`cfg_deer_brain_bt.tres` missing).
- P1.9 Commit 3 GREEN checks:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_deer_brain_bt.gd` passes (`3/3`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` passes (`2/2`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` passes (`3/3`).
- P1.9 Commit 4 RED check:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_rabbit_brain_bt.gd` fails for expected reason (`cfg_rabbit_brain_bt.tres` missing).
- P1.9 Commit 4 GREEN checks:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_rabbit_brain_bt.gd` passes (`3/3`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_deer_brain_bt.gd` passes (`3/3`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` passes (`2/2`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` passes (`3/3`).
- Current style check after P1.9 Commit 4:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` fails on pre-existing LOC cap guard (`S_AIBehaviorSystem` is `248` lines, limit `199`).
- Current full-suite check after P1.9 Commit 4:
  - `tools/run_gut_suite.sh` reports expected pre-P1.10 migration fallout (`4470` passing / `100` failing / `8` pending).
- P1.9 Commit 5 RED checks:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_patrol_drone_brain_bt.gd` fails for expected reason (`cfg_patrol_drone_brain_bt.tres` missing).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_sentry_brain_bt.gd` fails for expected reason (`cfg_sentry_brain_bt.tres` missing).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_guide_prism_brain_bt.gd` fails for expected reason (`cfg_guide_brain_bt.tres` missing).
- P1.9 Commit 5 GREEN checks:
...
- **BT runtime context keys**: `S_AIBehaviorSystem` now includes per-tick `delta` and `time` keys in BT context so `RS_BTAction`/`RS_BTCooldown` can run deterministically in gameplay and tests.
...
**Phase 3 (Docs Split)**: P3.0 reconciles ADR conventions + reshapes the `docs/` tree first. Inventory commit (P3.1), then one move commit per destination file (P3.3). `AGENTS.md` ends as a routing index (<150 lines). `DEV_PITFALLS.md` deleted when fully redistributed. **Extension recipes + decision ADRs require per-recipe user authorization** — they're authored at the tail of each owning phase, not batched in Phase 3. ADR renumbering scheme: V7.2 `0001` preserved; pre-existing `ADR-001..004` become `0002..0005`; V8 ADRs `0006..0010` (five total, not six — service-locator amendment in place of a separate `0007`).

**Phase 4 (Template/Demo)**: P4.1–P4.4 (scripts) complete — all scripts in `scripts/core/` or `scripts/demo/`. P4.5–P4.9 (resources/scenes/assets) extends the same core/demo split to `.tres` resources, `.tscn` scenes, and demo-only assets. P4.5 audit produces a detailed migration checklist. P4.6 moves ~170 core `.tres` files into `resources/core/` (demo resources already in `resources/demo/`). P4.7 splits `scenes/` into `scenes/core/` and `scenes/demo/`. P4.8 moves demo-only assets into `assets/demo/`. P4.9 adds enforcement tests for resource and scene boundaries. Each move commit updates imports + scene refs + `.tres` paths atomically.
...
1. **P4.5 Resources & Scenes Audit** — produce `docs/guides/cleanup_v8/resources_scenes_audit.md` enumerating every `.tres`/`.tscn`/asset file requiring move, with import/reference path impact analysis and commit sequence. Cross-reference `template_vs_demo.md` classification and `target_structure.md` target layout. Current state: `resources/demo/` has 107 files already moved; `resources/core/` is empty; `scenes/` is unsplit; `assets/` is unsplit.
2. **P4.6 Resources Core Move** — move ~170 core `.tres` files from flat `resources/` subdirs into `resources/core/`. One commit per logical chunk (base_settings, audio/ui, display, input, interactions, localization, qb, scene_director, scene_registry, spawn_metadata, state, textures, triggers, ui, ui_screens, ui_themes, vfx). Update all `preload()`/`load()` paths, `.tres` resource references, `.tscn` ext_resource paths, and `project.godot` autoload entries.
3. **P4.7 Scenes Split** — move core scenes into `scenes/core/` and demo scenes into `scenes/demo/`. Update all scene references and `project.godot` main scene entry.
4. **P4.8 Assets Demo Move** — move demo-only assets (2 models, 2 textures, 4 music tracks) into `assets/demo/`. Update all `.tscn` and `.tres` asset references.
5. **P4.9 Resources & Scenes Enforcement** — add `test_core_resources_never_reference_demo` and `test_core_scenes_never_reference_demo` to style suite. Verify full suite green, deletion test passes.
6. After Phase 4 complete: author remaining P3.5 recipes (`scenes.md`, `resources.md`) gated on Phase 4.
7. Keep docs/history references archived; new evergreen guidance belongs under `docs/guides/` or `docs/systems/`.
8. Run `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` after any file move.
