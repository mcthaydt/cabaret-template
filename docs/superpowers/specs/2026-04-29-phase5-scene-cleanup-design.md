...
Clean the template's scene tree to reflect its identity as a hybrid game template. One canonical builder-backed base scene (`tmpl_base_scene.tscn`), one clean blockout room as the demo entry point, and all legacy demo/temp scenes deleted.
...
- Extend `tmpl_base_scene.tscn` with hybrid-oriented Node3D container structure
...
### P5.2 — Canonical Base Scene (hybrid-Oriented)
...
| MODIFY | `scenes/core/templates/tmpl_base_scene.tscn` | P5.2 hybrid container structure |
...
| 4 | P5.2 | (GREEN) | Extend tmpl_base_scene.tscn with Node3D/hybrid containers |
...
- [ ] `docs/systems/hybrid/hybrid-template-pivot-plan.md` remains unchanged
