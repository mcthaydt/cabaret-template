...
This section documents key architectural choices that define how the StateStore system behaves.

### 1. Single Store Enforcement (Strict)

**Decision**: Only one StateStore instance allowed per scene tree. Multiple instances will self-destruct.

**Implementation**:
```gdscript
# In StateStore._ready()
func _ready():
...
**Decision**: Missing StateStore is a fatal error. Systems must assert store exists.
...
func _ready():
	...
	assert(store != null, "StateStore not found in scene tree. Add StateStore node to scene.")
	_store = store
```
...
**Decision**: StateStore placed in separate /Infrastructure branch, isolated from ECS and gameplay nodes.
...
**Decision**: All tests use real StateStore instances. No mocking.
...
**Save/Load**:
```gdscript
# StateStore filters by persistable flag
...
**Decision**: Calling get_store() before StateStore is ready is a fatal error.

**Implementation**:
```gdscript
# StateStoreUtils.get_store()
static func get_store(from_node: Node) -> StateStore:
	# Search logic...
	if store == null:
		assert(false, "StateStore not found. Ensure StateStore node exists in scene tree before accessing.")
	return store
```

**Recommendation**: Add StateStore to base scene template to ensure it's always present.
