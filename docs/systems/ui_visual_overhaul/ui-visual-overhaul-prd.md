...
- **Existing systems**: `U_DisplayUIThemeApplier` (`scripts/core/managers/helpers/display/u_display_ui_theme_applier.gd`) currently applies palette colors to UI roots independently. `U_PaletteManager` (`scripts/core/managers/helpers/u_palette_manager.gd`) and `RS_UIColorPalette` provide the color-blind palette system. Both are subsumed into the unified pipeline in Phase 0C.
