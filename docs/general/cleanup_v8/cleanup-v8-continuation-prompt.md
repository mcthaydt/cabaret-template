...
- **Phase 1**: IN PROGRESS.
  ...
  - **Verification state**:
    ...
    - Current full-suite check after P1.9 Commit 3 reports expected pre-P1.10 migration fallout (`4466` passing / `101` failing / `8` pending).
...
- P1.9 Commit 3 GREEN checks:
  ...
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_wolf_brain_bt.gd` passes (`2/2`).
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/ai/integration/test_s_ai_behavior_system_bt.gd` passes (`3/3`).
- Current style check after P1.9 Commit 3:
  - `tools/run_gut_suite.sh -gtest=res://tests/unit/style/test_style_enforcement.gd` fails on pre-existing LOC cap guard (`S_AIBehaviorSystem` is `248` lines, limit `199`).
- Current full-suite check after P1.9 Commit 3:
  - `tools/run_gut_suite.sh` reports expected pre-P1.10 migration fallout (`4466` passing / `101` failing / `8` pending).
...
2. Implement **P1.9 Commit 4 (RED+GREEN)** — rabbit BT parity test + rabbit BT brain resources.
3. Continue P1.9 creature migrations (sentry, patrol_drone, guide_prism) in order.
