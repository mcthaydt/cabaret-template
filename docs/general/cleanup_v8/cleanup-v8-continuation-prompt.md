...
**Next Task**: P1.9 Commit 3 (RED+GREEN) — add deer BT parity test and author `resources/ai/forest/deer/cfg_deer_brain_bt.tres`.
...
- **Phase 1**: IN PROGRESS.
  ...
  - **P1.9** in progress:
    ...
    - `(GREEN) P1.9 author wolf BT brain resources + runtime context follow-through` (working tree, commit pending)
  - **Verification state**:
...
2. Implement **P1.9 Commit 3 (RED+GREEN)** — deer BT parity test + `cfg_deer_brain_bt.tres`.
3. Continue P1.9 creature migrations (rabbit, sentry, patrol_drone, guide_prism) in order.
