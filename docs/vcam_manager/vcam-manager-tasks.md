...
**Latest completion note (March 15, 2026):** Phase 10C per-tick occlusion integration landed — `M_VCamManager` now gates occlusion by `vfx.occlusion_silhouette_enabled`, publishes disable/clear requests when silhouettes are disabled or transition ownership changes, and dispatches `vcam/update_silhouette_count` only on count changes. Validation is green for `tests/unit/managers/test_vcam_manager.gd` (`33/33`), `tests/unit/managers/test_vfx_manager_silhouette_routing.gd` (`3/3`), and style enforcement (`17/17`).
