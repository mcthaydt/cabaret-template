...
**Latest completion note (March 22, 2026):** Phase 12A observability validation is closed (`MT-40/41/42/47/48/49`) based on passing integration evidence (`tests/integration/vcam/test_vcam_state.gd` `9/9`, `test_vcam_occlusion.gd` `2/2`). Phase 13 automated regression/docs closure tasks are complete (`13.1/13.2/13.3/13.4/13.8`) with green gates across style (`17/17`), vCam runtime/systems/managers/helpers, camera-manager integration (`16/16`), and mobile/input regressions (`test_mobile_controls` `20/20`, `test_s_touchscreen_system` `8/8`, `test_input_system` `14/14`). Remaining Phase 13 scope is manual QA (`13.5/13.6/13.6b/13.7`).
