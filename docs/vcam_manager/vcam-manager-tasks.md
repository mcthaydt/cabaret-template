...
**Latest completion note (March 15, 2026):** Phase 10B2 silhouette routing landed — `EVENT_SILHOUETTE_UPDATE_REQUEST` constant plus `M_VCamManager` publish and `M_VFXManager` subscribe/delegate paths are now wired with gating coverage (`tests/unit/managers/test_vfx_manager_silhouette_routing.gd`, `3/3`) and manager publish coverage (`tests/unit/managers/test_vcam_manager.gd`, `28/28`); style gate remains fully green (`17/17`).
