...
**Latest completion note (March 21, 2026):** Phase 10C2 anti-flicker/stability landed — `U_VCamSilhouetteHelper` now applies occluders via `update_silhouettes(...)` using two-frame debounce and one-frame grace removal, with order-insensitive unchanged-set handling to prevent per-frame churn; `M_VFXManager` now routes silhouette payload updates through this API. Validation is green for `tests/unit/managers/helpers/test_vcam_silhouette_helper.gd` (`16/16`), `tests/unit/managers/test_vfx_manager_silhouette_routing.gd` (`5/5`), `tests/unit/managers/test_vfx_manager.gd` (`46/46`), `tests/unit/managers/test_vcam_manager.gd` (`49/49`), and style enforcement (`17/17`).
