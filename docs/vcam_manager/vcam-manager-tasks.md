...
**Latest planning note (March 15, 2026):** Phase 3 reset — OTS Phase 3C3 landed (`S_VCamSystem` event-driven OTS landing camera response + per-vCam `_ots_landing_response_state` dynamics + stack/no-op regression coverage in `test_vcam_system`). Next active target is 3C4 OTS aiming behavior.
