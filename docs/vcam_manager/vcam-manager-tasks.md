...
**Latest planning note (March 15, 2026):** Phase 3 reset — OTS Phase 3C2 landed (`S_VCamSystem` OTS shoulder sway + per-vCam `_shoulder_sway_state` dynamics + OTS/no-op regression coverage in `test_vcam_system`). Next active target is 3C3 landing camera response.
