...
**Created**: 2026-03-06
**Updated**: 2026-03-15
**Status**: Phases 0A-0F + 1A-1F + 2A-2B + 4A-4B + 5 + 6A + 6B + 6A2 + 6A.3 + 6A3a + 6A3b + 6A3c + Phase 8 core (`2C1/2C2/2C3/2C4/2C5/2C6/2C7/2C8/2C9/2C10/2C11`) + movement-style look smoothing + camera look smoothing parity + post-`0f51c36` retune doc/test catch-up + mobile drag-look/touch gating prerequisites + OTS Phase 3C1 collision avoidance complete (state/persistence + base authoring resources + dynamics + response tuning + mode resource/evaluator baselines + component/interface/manager core + `S_VCamSystem` baseline + runtime scene wiring + response-driven second-order smoothing integration + rotation continuity policy/tests + camera-state landing-impact scaffolding + QB-driven speed-FOV and landing-impact rule integration + orbit look-ahead/auto-level/soft-zone/hysteresis feel pass + ground-relative dual-anchor positioning + tuned orbit follow-bypass guard coverage + orbit release-smoothing enhancement + button-driven recenter interpolation + room-fade data-layer scaffolding + room-fade runtime logic/rendering + room-fade integration/polish validation + touch look dispatch + `touch_look_active` transient gating + input no-clobber guard + OTS collision distance clamping/recovery). Phase 3 reset is in progress: OTS Phases 3A/3B/3C1 are complete; next target is Phase 3C2 shoulder sway
...
| `ground_relative_enabled` | `bool` | `false` | 2C6 | Enables orbit dual-anchor vertical locking/re-anchor behavior |
| `ground_reanchor_min_height_delta` | `float` | `0.5` | 2C6 | Minimum landed terrain delta required before re-anchoring ground baseline |
| `ground_probe_max_distance` | `float` | `12.0` | 2C6 | Max downward probe distance used to resolve grounded reference height |
| `ground_anchor_blend_hz` | `float` | `4.0` | 2C6 | Dedicated blend frequency for ground-anchor target changes |
...
> Orbit follow-up backlog `2C11` and mobile drag-look/touch gating prerequisites are complete; OTS Phase 3C1 collision avoidance is now landed and the immediate implementation target is Phase 3C2 shoulder-sway work.
