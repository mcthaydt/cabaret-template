...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - vCam active-change observability contract (Phase 5C): on active-camera changes, `M_VCamManager` dispatches `U_VCamActions.set_active_runtime(...)` to the transient `vcam` slice and publishes `U_ECSEventNames.EVENT_VCAM_ACTIVE_CHANGED` through `U_ECSEventBus` with `{vcam_id, previous_vcam_id, mode}` payload.
