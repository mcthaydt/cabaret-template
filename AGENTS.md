...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - First-person mode resource contract: `RS_VCamModeFirstPerson` (`scripts/resources/display/vcam/rs_vcam_mode_first_person.gd`) is the authored baseline for first-person follow (`head_offset`, `look_multiplier`, `pitch_min`, `pitch_max`, `fov`) and aim transition settings (`aim_blend_duration`, `aim_exit_blend_duration`); consumers should read through `get_resolved_values()` for clamp/order safety.
  - Mode evaluator contract (Refactor Phase 1C): `U_VCamModeEvaluator.evaluate(...)` supports orbit + first-person only and returns `{transform, fov, mode_name}` for valid inputs; null/invalid mode/target inputs must return `{}` without warning-channel noise.
  ...
  - Aim activation contract (Refactor Phase 1A): `S_VCamSystem` reads `input.aim_pressed` from Redux and handles hold-to-aim switching via `_process_aim_activation(...)`; press selects a first-person vCam via `_find_aim_target_fp_vcam_id(...)`, release restores `_aim_restore_vcam_id`, and enter/exit transitions must use authored first-person `aim_blend_duration` / `aim_exit_blend_duration` (min-clamped, no hardcoded blend timings).
