...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - Orbit response-feel contract (Phase 2C1/2C2): `RS_VCamResponse` now also carries orbit look-ahead/auto-level tuning (`look_ahead_distance`, `look_ahead_smoothing`, `auto_level_speed`, `auto_level_delay`). `S_VCamSystem` applies look-ahead as a pre-smoothing position offset and applies delayed auto-level pitch recenter only for orbit mode.
  ...
  - First-person mode resource contract (Phase 3A): `RS_VCamModeFirstPerson` (`scripts/resources/display/vcam/rs_vcam_mode_first_person.gd`) is the authored baseline for first-person (`head_offset`, `look_multiplier`, `pitch_min`, `pitch_max`, `fov`); `get_resolved_values()` is the canonical clamp/order helper for evaluator/runtime reads.
  ...
  - Mode evaluator contract (Phase 4B): `U_VCamModeEvaluator.evaluate(...)` returns `{transform, fov, mode_name}` for supported modes and must return `{}` for null/invalid mode/target inputs without warning-channel noise. First-person pitch clamping is evaluator-owned; `look_multiplier` remains system-owned. Fixed mode must ignore runtime yaw/pitch, and `use_path` forces anchor-basis facing (track-target ignored).
  ...
  - vCam response smoothing contract (Phase 6A2 + look-smoothing follow-up): `S_VCamSystem` applies `RS_VCamResponse` smoothing per vCam with `U_SecondOrderDynamics3D` position smoothing for all modes, while rotation smoothing on this path is fixed-mode-only to avoid double-softness with orbit/first-person look smoothing. Smoothing state is keyed by `vcam_id`, recreated on response tuning changes, reset on mode/follow-target changes, and bypassed entirely when `C_VCamComponent.response` is `null` (raw evaluator passthrough). Fixed-mode rotation smoothing targets must unwrap against prior targets to avoid long-path angle spins near `-PI/PI` boundaries.
  - vCam look smoothing contract (Movement-style follow-up, March 2026): orbit/first-person evaluator rotation consumes per-vCam spring-damper look state (`smoothed_yaw`, `smoothed_pitch`, `yaw_velocity`, `pitch_velocity`) using response `rotation_frequency` + `rotation_damping`; `C_VCamComponent.runtime_yaw`/`runtime_pitch` remain raw target values for continuity/contracts, and look state resets on mode/target/response changes plus vCam prune/clear.
  ...
  - Touch look gating contract (planned): `gameplay.touch_look_active` Redux flag will gate `S_InputSystem` look dispatch once mobile drag-look is implemented. Until then, treat the flag path as pending and keep `S_TouchscreenSystem`/`UI_MobileControls` drag-look work as required for orbit/first-person mobile parity.
