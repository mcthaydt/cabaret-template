...
- QB Rule Engine v2 patterns (Phase 5 complete)
  ...
  - Rule assets use `RS_Rule` + typed condition/effect resources (`RS_Condition*`, `RS_Effect*`). `conditions` and `effects` use typed arrays (`Array[I_Condition]`, `Array[I_Effect]`) with F7-style coerce setters; `U_RuleValidator` validates semantic correctness (required fields, valid ranges) as a double-check layer.
...
- AI behavior-tree loop pattern (M7 + P1.10 complete)
  - `S_AIBehaviorSystem` (`scripts/ecs/systems/s_ai_behavior_system.gd`) is the canonical BT orchestration system. It evaluates brain roots through `U_BTRunner`, assembles context via `U_AIContextAssembler`, and keeps debug output throttled through `U_DebugLogThrottle`.
  - Orchestration split contract (R3/R10 follow-through): keep `S_AIBehaviorSystem` orchestration-first (no monolithic inline selector/planner/task-runner stacks), with BT planning isolated to `RS_BTPlanner` + `U_BTPlannerSearch` and world-state assembly isolated to `U_AIWorldStateBuilder`.
  ...
  - Typed-contract pattern (R1 follow-through): treat AI brain/BT/action fields as strongly typed runtime contracts, not duck-typed payloads. Canonical types are `C_AIBrainComponent.brain_settings: RS_AIBrainSettings`, `RS_AIBrainSettings.root: RS_BTNode`, BT resources under `scripts/resources/bt/` + `scripts/resources/ai/bt/`, and action payloads implementing `I_AIAction`.
  - AI resource directory contract (R7 follow-through): keep core AI resource scripts organized under `scripts/resources/ai/brain/`, `scripts/resources/ai/bt/`, and `scripts/resources/ai/actions/`. Legacy `scripts/resources/ai/goals/` and `scripts/resources/ai/tasks/` are removed by P1.10 and must not be reintroduced.
  - Evaluation-throttle contract: honor `RS_AIBrainSettings.evaluation_interval` using `C_AIBrainComponent.evaluation_timer`; first evaluation should run immediately for brains without an active goal. Running BT actions must continue ticking every physics frame while evaluation gating is active (interval throttles re-evaluation only when there is no running BT state).
  - BT action context contract (woods verification): `U_AIContextAssembler.build_context(...)` must inject the active scene `ecs_manager` into context so scan/reserve/harvest/deposit/build actions can resolve authored ECS targets.
  - AI action position contract (woods verification): movement-sensitive actions must resolve actor/target positions through `U_AIActionPositionResolver` before falling back to entity roots, because scene-authored character roots may stay stationary while child `CharacterBody3D` nodes move.
  - Harvest ordering contract (woods remediation): `RS_AIActionHarvest` must validate inventory acceptance before decrementing resource-node stock and clear owned reservations after harvest attempts (success or rejection) to prevent reservation deadlocks and silent resource loss.
  - Move-to-nearest filtering contract (woods remediation): `RS_AIActionMoveToNearest` supports optional authored filters `scan_required_resource_type` and `scan_required_harvest_tag`; builder gather branches should set these so gather_wood targets wood nodes only.
  - BT runner contract (P1.8/P1.10): action lifecycle is driven by `RS_BTAction` + `U_BTRunner` (`I_AIAction.start/tick/is_complete`) and per-node runtime state lives in `C_AIBrainComponent.bt_state_bag`, keyed by BT node instance IDs.
  ...
  - Movement/stub action contract (M7/M12/R6): `RS_AIActionMoveTo` resolves waypoint/node/position targets and writes `C_MoveTargetComponent` runtime fields (`target_position`, `arrival_threshold`, `is_active`); `RS_AIActionScan` tracks scan timing in task state; `RS_AIActionAnimate` is a stub that sets `task_state["animation_state"]` and completes immediately.
  - Move-target follower contract (R6 + P1.10): `S_MoveTargetFollowerSystem` (`scripts/ecs/systems/s_move_target_follower_system.gd`, `execution_priority = -5`) is the canonical world-space move-vector bridge. It queries `C_InputComponent` + `C_MovementComponent` + `C_MoveTargetComponent` and consumes active move-target payloads only (no legacy GOAP task-state fallback path).
  ...
  - Demo trigger gating contract (M10 audit follow-up): do not gate BT utility branches off transient one-frame input fields (for example `camera_center_just_pressed`). Use durable gameplay flags under `gameplay.ai_demo_flags.*` and drive them from authored trigger zones (`Inter_AIDemoFlagZone`) so evaluate-interval scheduling cannot miss triggers.
...
### UI Theme and Motion Pipelines

- UI manager, theme, motion, registry, base-class, and settings-panel patterns → `docs/systems/ui_manager/ui-manager-overview.md`
...
- Character lighting manager patterns → `docs/systems/lighting_manager/lighting-manager-overview.md`
...
- Scene Manager architecture, transition, overlay, trigger, spawn, camera-blend, cache, and pitfall guidance → `docs/systems/scene_manager/scene-manager-overview.md`
