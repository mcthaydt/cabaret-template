...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - Mode evaluator contract (Phase 2B): `U_VCamModeEvaluator.evaluate(...)` returns `{transform, fov, mode_name}` for supported modes and must return `{}` for null/invalid mode/target inputs without warning-channel noise.
