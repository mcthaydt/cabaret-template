...
- AI goal-loop pattern (M7 complete)
  ...
  - Movement/stub action contract (M7/M12): `RS_AIActionMoveTo` resolves waypoint/node/position targets and writes `task_state["ai_move_target"]` plus `task_state["ai_arrival_threshold"]`; `RS_AIActionScan` tracks scan timing in task state; `RS_AIActionAnimate` is a stub that sets `task_state["animation_state"]` and completes immediately.
  - AI navigation bridge contract (M12): `S_AINavigationSystem` (`scripts/ecs/systems/s_ai_navigation_system.gd`, `execution_priority = -5`) reads `task_state["ai_move_target"]` and optional `task_state["ai_arrival_threshold"]`, computes XZ world direction, and writes world-space `C_InputComponent.set_move_vector()` (`Vector2(direction.x, direction.z)`) without camera-basis conversion.
  ...
  - NPC prefab unification contract (M13): authored demo NPC entities should instance `scenes/prefabs/prefab_npc.tscn` (inherits `tmpl_character.tscn`) and override only archetype-specific fields (`entity_id`, tags, brain settings, visuals). Runtime body path is `Player_Body` (not legacy `NPC_Body`), and custom CSG visuals under NPC bodies must keep `use_collision = false`.
  ...
  - AI spawn-recovery contract (M11 hardening): use explicit authored spawn points (via `RS_AIBrainSettings.respawn_spawn_point_id`) for floating AI recovery; do not depend on last-supported pose caches. `S_AISpawnRecoverySystem` tracks unsupported duration/cooldown, calls `I_SpawnManager.spawn_entity_at_point(scene, entity_id, spawn_point_id)`, clears only AI `task_state` + input/velocity, and disables recovery for the entity session when the configured spawn point is missing.
