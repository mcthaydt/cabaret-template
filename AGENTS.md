...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - vCam response smoothing contract (Phase 6A2): `S_VCamSystem` applies `RS_VCamResponse` smoothing per vCam (`U_SecondOrderDynamics3D` for position + per-axis `U_SecondOrderDynamics` for rotation). Smoothing state is keyed by `vcam_id`, recreated on response tuning changes, reset on mode/follow-target changes, and bypassed entirely when `C_VCamComponent.response` is `null` (raw evaluator passthrough). Rotation smoothing targets must unwrap against prior targets to avoid long-path angle spins near `-PI/PI` boundaries.
