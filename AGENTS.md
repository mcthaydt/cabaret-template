...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - First-person mode resource contract (Phase 3A + Phase 9/3C1): `RS_VCamModeFirstPerson` (`scripts/resources/display/vcam/rs_vcam_mode_first_person.gd`) is the authored baseline for first-person (`head_offset`, `look_multiplier`, `pitch_min`, `pitch_max`, `fov`, `strafe_tilt_angle`, `strafe_tilt_smoothing`); `get_resolved_values()` is the canonical clamp/order helper for evaluator/runtime reads.
  ...
  - Orbit release-smoothing contract (Phase 2C7, March 2026): `RS_VCamResponse` carries `look_release_yaw_damping`, `look_release_pitch_damping`, and `look_release_stop_threshold`. `S_VCamSystem` applies these only for orbit no-input ticks by damping existing look-smoothing velocities and clamping near-zero residual velocity to prevent drift; first-person/fixed release behavior remains unchanged.
  - First-person strafe-tilt contract (Phase 9/3C1, March 2026): `S_VCamSystem` reads shared `input.move_input` and applies first-person-only roll target (`move_input.x * strafe_tilt_angle`) after evaluator yaw/pitch construction, smoothed through per-vCam `U_SecondOrderDynamics` state in `_first_person_strafe_tilt_state`; clear/reset that state when mode is not first-person, when authored angle is disabled, and during stale-vCam prune/clear.
