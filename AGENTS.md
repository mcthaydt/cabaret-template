...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - OTS mode resource contract (Phase 3A): `RS_VCamModeOTS` (`scripts/resources/display/vcam/rs_vcam_mode_ots.gd`) is the authored baseline for OTS over-the-shoulder (`shoulder_offset`, `camera_distance`, `look_multiplier`, `pitch_min`, `pitch_max`, `fov`, `collision_probe_radius`, `collision_recovery_speed`, `shoulder_sway_angle`, `shoulder_sway_smoothing`, `landing_dip_distance`, `landing_dip_recovery_speed`); `get_resolved_values()` is the canonical clamp/order helper for evaluator/runtime reads.
