...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - Mode evaluator contract (Phase 3B): `U_VCamModeEvaluator.evaluate(...)` returns `{transform, fov, mode_name}` for supported modes and must return `{}` for null/invalid mode/target inputs without warning-channel noise. First-person pitch clamping is evaluator-owned; `look_multiplier` remains system-owned (applied in `S_VCamSystem`, not evaluator helpers).
