...
- vCam Runtime Contracts (Documentation Sweep 2026-03)
  ...
  - Occlusion silhouette-helper contract (Phase 10B): `U_VCamSilhouetteHelper` (`scripts/managers/helpers/u_vcam_silhouette_helper.gd`) is the canonical material-override helper for occlusion silhouettes; it must preserve original material state, apply deterministic shader overrides, restore via `remove_silhouette`/`remove_all_silhouettes`, and safely no-op on invalid/freed targets.
  ...
  - OTS mode resource contract (Phase 3A + 3C4): `RS_VCamModeOTS` (`scripts/resources/display/vcam/rs_vcam_mode_ots.gd`) is the authored baseline for OTS over-the-shoulder (`shoulder_offset`, `camera_distance`, `look_multiplier`, `pitch_min`, `pitch_max`, `fov`, `collision_probe_radius`, `collision_recovery_speed`, `shoulder_sway_angle`, `shoulder_sway_smoothing`, `landing_dip_distance`, `landing_dip_recovery_speed`, `movement_profile`, `disable_sprint`, `lock_facing_to_camera`, `aim_blend_duration`); `get_resolved_values()` is the canonical clamp/order helper for evaluator/runtime reads.
  - Fixed mode resource contract (Phase 4A): `RS_VCamModeFixed` (`scripts/resources/display/vcam/rs_vcam_mode_fixed.gd`) is the authored baseline for fixed mode (`use_world_anchor`, `track_target`, `fov`, `tracking_damping`, `follow_offset`, `use_path`, `path_max_speed`, `path_damping`); damping/speed fields are system-owned and should not be consumed in evaluator helpers.
  - Mode evaluator contract (Phase 4B): `U_VCamModeEvaluator.evaluate(...)` returns `{transform, fov, mode_name}` for supported modes and must return `{}` for null/invalid mode/target inputs without warning-channel noise. OTS pitch clamping is evaluator-owned; `look_multiplier` remains system-owned. Fixed mode must ignore runtime yaw/pitch, and `use_path` forces anchor-basis facing (track-target ignored).
  ...
  - vCam target/path resolution contract (Phase 6A): follow-target resolution priority is NodePath (`follow_target_path`) -> entity ID (`follow_target_entity_id`) -> tag (`follow_target_tag`). Multiple valid tag matches use the first ECS-registration-order entity and emit a debug issue. Fixed `use_path` cameras create/keep `PathFollow3D` helpers under the gameplay `Path3D` node (never under persistent root managers); invalid path-target ticks should not fabricate new path progress.
  - vCam response smoothing contract (Phase 6A2 + look-smoothing follow-up): `S_VCamSystem` applies `RS_VCamResponse` smoothing per vCam with `U_SecondOrderDynamics3D` position smoothing for all modes, while rotation smoothing on this path is fixed-mode-only to avoid double-softness with orbit/OTS look smoothing. Smoothing state is keyed by `vcam_id`, recreated on response tuning changes, reset on mode/follow-target changes, and bypassed entirely when `C_VCamComponent.response` is `null` (raw evaluator passthrough). Fixed-mode rotation smoothing targets must unwrap against prior targets to avoid long-path angle spins near `-PI/PI` boundaries.
  - vCam look smoothing contract (Movement-style follow-up, March 2026): orbit/OTS evaluator rotation consumes per-vCam spring-damper look state (`smoothed_yaw`, `smoothed_pitch`, `yaw_velocity`, `pitch_velocity`) using response `rotation_frequency` + `rotation_damping`; `C_VCamComponent.runtime_yaw`/`runtime_pitch` remain raw target values for continuity/contracts, and look state resets on mode/target/response changes plus vCam prune/clear.
  ...
  - Orbit release-smoothing contract (Phase 2C7, March 2026): `RS_VCamResponse` carries `look_release_yaw_damping`, `look_release_pitch_damping`, and `look_release_stop_threshold`. `S_VCamSystem` applies these only for orbit no-input ticks by damping existing look-smoothing velocities and clamping near-zero residual velocity to prevent drift; OTS/fixed release behavior remains unchanged.
  - OTS collision avoidance contract (Phase 9/3C1): `S_VCamSystem` spherecasts from follow target toward desired OTS camera position using gameplay `World3D` physics space; on hit, clamp camera distance to `hit_distance - collision_probe_radius` with minimum distance floor; run an initial-overlap `intersect_shape` guard before sweep so near-wall overlaps clamp correctly; smooth recovery via per-vCam `U_SecondOrderDynamics` at `collision_recovery_speed` Hz in `_ots_collision_state`; clear/reset that state when mode is not OTS and during stale-vCam prune/clear.
  - OTS shoulder sway contract (Phase 9/3C2): `S_VCamSystem` reads shared `input.move_input` and applies OTS-only roll target (`move_input.x * shoulder_sway_angle`) after evaluator yaw/pitch construction, smoothed through per-vCam `U_SecondOrderDynamics` state in `_shoulder_sway_state`; clear/reset that state when mode is not OTS, when authored angle is disabled, and during stale-vCam prune/clear.
  - OTS landing-response contract (Phase 9/3C3): `S_VCamSystem` subscribes to `U_ECSEventNames.EVENT_ENTITY_LANDED`, normalizes player landing fall speed (`5..30` -> `0..1`) from payload (`fall_speed` preferred, `vertical_velocity`/`velocity.y` fallback), and applies OTS-only distance compression (`landing_dip_distance * normalized`) through per-vCam `_ots_landing_response_state` (`U_SecondOrderDynamics` at `landing_dip_recovery_speed`). Apply compression along the OTS cast direction after collision/smoothing, keep `OTS_MIN_CAMERA_DISTANCE` floor, stack with shared `landing_impact_offset`, and clear/reset state on non-OTS/disabled/stale paths.
  - OTS aiming activation contract (Phase 3C4 slice 1, March 2026): `S_VCamSystem` reads `input.aim_pressed` from Redux and handles hold-to-aim camera switching via `_process_aim_activation(...)`; enter selects OTS cameras via `_find_aim_target_ots_vcam_id(...)` (prefer matching follow target, then priority/id tie-break), release restores `_aim_restore_vcam_id`, and both transitions must use authored `aim_blend_duration` from `RS_VCamModeOTS` (min-clamped, no hardcoded blend).
  - OTS movement-profile contract (Phase 3C4 slice 1, March 2026): `S_MovementSystem` resolves active OTS mode via `state.vcam.active_vcam_id` + `C_VCamComponent` lookup. OTS with `movement_profile` overrides per-tick movement settings immediately (no interpolation), and `disable_sprint` must gate sprint before max-speed calculation.
  - OTS facing-lock contract (Phase 3C4 slice 1, March 2026): `S_RotateToInputSystem` resolves active OTS mode via `state.vcam.active_vcam_id` + `C_VCamComponent` lookup; with `lock_facing_to_camera = true`, desired yaw follows active camera yaw and the system must not early-reset rotation state on zero move input while lock is active. Non-OTS paths remain unchanged.
  - OTS reticle contract (Phase 3C4.9/3C4.10, March 2026): `UI_HudController` owns `OTSReticleContainer` visibility/fade and must gate display to unpaused gameplay while `state.vcam.active_mode == "ots"`. Fade timing must resolve from authored OTS `aim_blend_duration` when available (fallback `0.15s`), and reticle must stay hidden by default outside OTS.
  - OTS default movement preset contract (Phase 3C4.11, March 2026): `resources/base_settings/gameplay/cfg_ots_movement_default.tres` is the canonical default `RS_MovementSettings` profile for OTS reduced-speed movement, and `resources/display/vcam/cfg_default_ots.tres` must reference it via `movement_profile`.
  ...
  - vCam rotation continuity contract (Phase 6A.3): `S_VCamSystem` applies runtime yaw/pitch carry/reset/reseed policy on active-vCam switches before evaluation. Orbit↔OTS carries yaw and resets pitch to `0.0`; fixed→orbit/OTS reseeds to incoming authored yaw with pitch reset; same-mode switches carry yaw/pitch only when follow targets resolve to the same node, otherwise reseed to incoming authored angles.
  ...
  - Touch aim contract (Phase 3C4 slice 1, March 2026): `UI_MobileControls` owns empty-space long-press aim toggling and exposes `consume_aim_pressed()`. `S_TouchscreenSystem` must dispatch `U_InputActions.update_aim_state(...)` from this consume API (never hardcode `false`), and `S_InputSystem` / input sources must keep `aim_pressed` in the shared input payload contract.
  - Silhouette routing contract (Phase 10B2): `M_VCamManager` publishes `EVENT_SILHOUETTE_UPDATE_REQUEST` from active-camera submission using `U_VCamCollisionDetector` results and payload `{entity_id, occluders, enabled}`; `M_VFXManager` is the sole subscriber/applicator and must gate requests with `_is_player_entity()` + `_is_transition_blocked()` before delegating to `U_VCamSilhouetteHelper`.
  - Per-tick occlusion integration contract (Phase 10C): `M_VCamManager` must consult `U_VFXSelectors.is_occlusion_silhouette_enabled(...)` before occluder detection, publish `enabled=false` clear requests when silhouettes are disabled or transition blend ownership blocks gameplay camera submission, and dispatch `U_VCamActions.update_silhouette_count(...)` only when the count changes (clear to `0` on runtime teardown paths such as last-vCam unregister).
