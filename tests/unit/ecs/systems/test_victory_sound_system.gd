...
const VICTORY_SOUND_SYSTEM := preload("res://scripts/core/ecs/systems/s_victory_sound_system.gd")
const VICTORY_SOUND_SETTINGS := preload("res://scripts/core/resources/ecs/rs_victory_sound_settings.gd")
...
const TEST_EVENT := StringName("victory_triggered")
...
func _make_settings(stream: AudioStream) -> Resource:
	var settings := VICTORY_SOUND_SETTINGS.new()
...
func test_system_extends_ecs_system() -> void:
	var system := VICTORY_SOUND_SYSTEM.new()
	autofree(system)
	assert_true(system is BaseECSSystem)

func test_get_event_name_is_victory_triggered() -> void:
	var system := VICTORY_SOUND_SYSTEM.new()
	autofree(system)
	assert_eq(system.get_event_name(), TEST_EVENT)

func test_event_queues_request_with_body_position() -> void:
	...
	add_child(body)
	autofree(body)
	body.global_position = Vector3(1, 2, 3)

	EVENT_BUS.publish(TEST_EVENT, {"body": body})
...
func test_victory_spawns_sound_at_body_position() -> void:
	var manager := _spawn_manager()
	await _pump()
	...
	add_child(body)
	autofree(body)
	body.global_position = Vector3(4, 5, 6)
	...
	assert_eq(used[0].global_position, Vector3(4, 5, 6))
