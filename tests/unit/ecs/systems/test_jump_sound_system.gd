...
const JUMP_SOUND_SYSTEM := preload("res://scripts/core/ecs/systems/s_jump_sound_system.gd")
const JUMP_SOUND_SETTINGS := preload("res://scripts/core/resources/ecs/rs_jump_sound_settings.gd")
...
const TEST_EVENT := StringName("entity_jumped")
...
func _make_settings(stream: AudioStream) -> Resource:
	var settings := JUMP_SOUND_SETTINGS.new()
...
func test_system_extends_ecs_system() -> void:
	var system := JUMP_SOUND_SYSTEM.new()
	autofree(system)
	assert_true(system is BaseECSSystem)

func test_get_event_name_is_entity_jumped() -> void:
	var system := JUMP_SOUND_SYSTEM.new()
	autofree(system)
	assert_eq(system.get_event_name(), TEST_EVENT)

func test_event_queues_request_with_position_and_jump_force() -> void:
	...
	EVENT_BUS.publish(TEST_EVENT, {"position": position, "jump_force": 12.0})

	assert_eq(system.requests.size(), 1)
	assert_eq(system.requests[0].get("position"), position)
	assert_eq(system.requests[0].get("jump_force"), 12.0)
...
func test_process_tick_with_null_settings_clears_requests_and_spawns_nothing() -> void:
	...
	EVENT_BUS.publish(TEST_EVENT, {"position": Vector3.ZERO, "jump_force": 10.0})
	assert_eq(system.requests.size(), 1)
...
func test_process_tick_spawns_sfx_for_each_request() -> void:
	var manager := _spawn_manager()
	await _pump()
	...
	EVENT_BUS.publish(TEST_EVENT, {"position": Vector3(1, 0, 0), "jump_force": 10.0})
	assert_eq(system.requests.size(), 1)
...
func test_min_interval_prevents_multiple_spawns_in_same_tick() -> void:
	...
	EVENT_BUS.publish(TEST_EVENT, {"position": Vector3.ZERO, "jump_force": 10.0})
	EVENT_BUS.publish(TEST_EVENT, {"position": Vector3.ZERO, "jump_force": 10.0})
	assert_eq(system.requests.size(), 2)
...
func test_requests_are_cleared_after_processing() -> void:
	var manager := _spawn_manager()
	await _pump()
