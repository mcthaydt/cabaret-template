...
# Test 16: is_occlusion_silhouette_enabled with valid state (enabled)
func test_is_occlusion_silhouette_enabled_returns_true() -> void:
	var state := _make_state(true, 1.0, true, true, true)
	var result: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)

	assert_true(
		result,
		"is_occlusion_silhouette_enabled should return true when enabled"
	)

# Test 17: is_occlusion_silhouette_enabled with valid state (disabled)
func test_is_occlusion_silhouette_enabled_returns_false() -> void:
	var state := _make_state(true, 1.0, true, true, false)
	var result: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)

	assert_false(
		result,
		"is_occlusion_silhouette_enabled should return false when disabled"
	)

# Test 18: is_occlusion_silhouette_enabled with missing vfx slice
func test_is_occlusion_silhouette_enabled_missing_vfx_slice() -> void:
	var state := {}
	var result: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)

	assert_true(
		result,
		"is_occlusion_silhouette_enabled should return true (default) when vfx slice missing"
	)

# Test 19: is_occlusion_silhouette_enabled with missing field
func test_is_occlusion_silhouette_enabled_missing_field() -> void:
	var state := {"vfx": {"screen_shake_enabled": true}}
	var result: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)

	assert_true(
		result,
		"is_occlusion_silhouette_enabled should return true (default) when field missing"
	)

# Test 20: Selectors are pure functions (same input = same output)
func test_selectors_are_pure_functions() -> void:
	...
	var result1_silhouette: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)
	var result2_silhouette: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(state)
	...
	assert_eq(
		result1_silhouette,
		result2_silhouette,
		"is_occlusion_silhouette_enabled should return same result for same input"
	)

# Test 21: Selectors do not mutate state
func test_selectors_do_not_mutate_state() -> void:
	...
	var _silhouette: bool = U_VFXSelectors.is_occlusion_silhouette_enabled(original_state)
