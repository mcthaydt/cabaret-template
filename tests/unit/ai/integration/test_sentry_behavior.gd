...
const BUILDER_SCRIPT_PATH := "res://scripts/demo/ai/trees/sentry_behavior.gd"
...
func _build_tree() -> RS_BTNode:
	...
	assert_true(builder.has_method("build"), "sentry_behavior should have build() method")
...
func test_sentry_behavior_builder_exists() -> void:
	assert_true(
		FileAccess.file_exists(BUILDER_SCRIPT_PATH),
		"sentry_behavior.gd should exist at scripts/demo/ai/trees/"
	)

func test_sentry_builder_returns_utility_selector_root() -> void:
	var root: RS_BTNode = _build_tree()
	if root == null:
		return
	assert_not_null(
		root as RS_BTUtilitySelector,
		"sentry root should be an RS_BTUtilitySelector"
	)

func test_sentry_builder_root_has_three_scorer_child_pairs() -> void:
	var root: RS_BTNode = _build_tree()
	if root == null or not (root is RS_BTUtilitySelector):
		return
	var selector: RS_BTUtilitySelector = root as RS_BTUtilitySelector
	assert_eq(selector.child_scorers.size(), 3, "Sentry BT should have 3 scorers")
	assert_eq(selector.children.size(), 3, "Sentry BT should have 3 children")

func test_sentry_builder_first_scorer_if_true_is_eleven() -> void:
	var root: RS_BTNode = _build_tree()
	if root == null or not (root is RS_BTUtilitySelector):
		return
	var selector: RS_BTUtilitySelector = root as RS_BTUtilitySelector
	...
	assert_true(
		...
		"First scorer should be RS_AIScorerCondition (disturbance_proximity)"
	)
	...
	assert_eq(
		...
		11.0,
		"Disturbance-proximity scorer should have if_true=11.0"
	)

func test_sentry_builder_last_scorer_is_constant_one() -> void:
	var root: RS_BTNode = _build_tree()
	if root == null or not (root is RS_BTUtilitySelector):
		return
	var selector: RS_BTUtilitySelector = root as RS_BTUtilitySelector
	...
	assert_true(
		...
		"Last scorer should be RS_AIScorerConstant (guard fallback)"
	)
	if not (scorer is RS_AIScorerConstant):
		return
	var const_scorer: RS_AIScorerConstant = scorer as RS_AIScorerConstant
	assert_eq(const_scorer.value, 1.0, "Guard fallback scorer should have value=1.0")
