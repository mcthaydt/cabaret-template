...
const RABBIT_BT_BRAIN_PATH := "res://resources/ai/forest/rabbit/cfg_rabbit_brain_bt.tres"
...
func _create_fixture(predator_detected: bool, hunger: float) -> Dictionary:
	...
	var brain_settings: RS_AIBrainSettings = _load_required_brain_settings(RABBIT_BT_BRAIN_PATH)
	...
	var rabbit_entity: BaseECSEntity = BASE_ECS_ENTITY.new()
	rabbit_entity.name = "E_RabbitBT"
	rabbit_entity.entity_id = &"rabbit_bt"
	rabbit_entity.tags = [&"prey", &"ai", &"forest"]
	root.add_child(rabbit_entity)
	autofree(rabbit_entity)

	var body := CharacterBody3D.new()
	body.name = "RabbitBody"
	rabbit_entity.add_child(body)
	autofree(body)

	var brain: C_AIBrainComponent = C_AI_BRAIN_COMPONENT.new()
	brain.brain_settings = brain_settings
	rabbit_entity.add_child(brain)
	autofree(brain)
	manager.add_component_to_entity(rabbit_entity, brain)
	...
	rabbit_entity.add_child(detection)
	autofree(detection)
	manager.add_component_to_entity(rabbit_entity, detection)
	...
	rabbit_entity.add_child(needs)
	autofree(needs)
	await get_tree().process_frame
	needs.hunger = hunger
	manager.add_component_to_entity(rabbit_entity, needs)

	var move_target: C_MoveTargetComponent = C_MOVE_TARGET_COMPONENT.new()
	rabbit_entity.add_child(move_target)
	autofree(move_target)
	manager.add_component_to_entity(rabbit_entity, move_target)

	var movement: C_MovementComponent = C_MOVEMENT_COMPONENT.new()
	movement.settings = RS_MOVEMENT_SETTINGS.new()
	rabbit_entity.add_child(movement)
	autofree(movement)
	manager.add_component_to_entity(rabbit_entity, movement)
...
func test_rabbit_threat_response_flees_without_startle() -> void:
	...
	assert_true(
		...
		"Threatened rabbit should immediately start flee action with a move target."
	)
	assert_false(
		_brain_state_has_action_key(brain, U_AI_TASK_STATE_KEYS.SCAN_ELAPSED),
		"Rabbit threat response should not run deer-style startle scan before fleeing."
	)
	assert_true(move_target.is_active, "Flee action should leave move-target component active while escaping.")

func test_rabbit_without_threat_graze_sequence_increases_hunger_when_hungry() -> void:
	...
	assert_true(observed_wait, "Hungry rabbit should enter graze wait action before feed.")
...
func test_rabbit_without_threat_uses_wander_when_sated() -> void:
	...
	var observed_wander: bool = _observe_action_key_over_ticks(
		system,
		brain,
	...
	assert_true(observed_wander, "Sated rabbit should fall back to wander behavior.")
	assert_true(move_target.is_active, "Wander action should drive an active move-target request.")
