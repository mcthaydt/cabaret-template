...
func test_silhouette_event_is_processed_through_vfx_manager_routing() -> void:
	...
	assert_true(target.material_override is ShaderMaterial, "VFX manager should apply silhouette override from event payload")
	...
	assert_eq(target.material_override, original_material, "Disable request should restore original material override")

func test_silhouette_event_respects_player_gating() -> void:
	var manager := await _create_manager_with_state(StringName("player"), false, [], StringName("gameplay"))
	var target := _create_mesh_target()
	target.material_override = StandardMaterial3D.new()
	var original_material: Material = target.material_override
	...
	assert_eq(target.material_override, original_material, "Non-player silhouette request should be ignored")
...
func test_silhouette_event_respects_transition_blocking() -> void:
	...
	assert_eq(target.material_override, original_material, "Transition-blocked silhouette request should be ignored")
