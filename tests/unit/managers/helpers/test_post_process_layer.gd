...
func before_each() -> void:
	...
	_create_effect_rects(_layer)

func test_initialize_caches_effect_rects() -> void:
	_helper.initialize(_layer)
...
func test_initialize_handles_null_canvas_layer() -> void:
	...
	assert_null(
		_helper.get_effect_rect(U_PostProcessLayer.EFFECT_FILM_GRAIN),
		"Null canvas layer should not cache rects"
	)
...
func _create_effect_rects(layer: CanvasLayer) -> void:
	var names := ["FilmGrainRect", "OutlineRect", "DitherRect", "LUTRect"]
	for name in names:
