...
## Unit tests for U_ColorGradingSelectors
##
## Validates:
...
func test_filter_mode_reads_from_state() -> void:
	var state := {"display": {"color_grading_filter_mode": 6}}
	assert_eq(U_COLOR_GRADING_SELECTORS.get_filter_mode(state), 6,
		"filter_mode should read 6 from state")


func test_filter_intensity_reads_from_state() -> void:
	var state := {"display": {"color_grading_filter_intensity": 0.47}}
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_filter_intensity(state), 0.47, 0.001,
		"filter_intensity should read 0.47 from state")


func test_exposure_reads_from_state() -> void:
	var state := {"display": {"color_grading_exposure": -0.18}}
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_exposure(state), -0.18, 0.001,
		"exposure should read -0.18 from state")


func test_contrast_reads_from_state() -> void:
	var state := {"display": {"color_grading_contrast": 1.23}}
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_contrast(state), 1.23, 0.001,
		"contrast should read 1.23 from state")


func test_saturation_reads_from_state() -> void:
	var state := {"display": {"color_grading_saturation": 1.37}}
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_saturation(state), 1.37, 0.001,
		"saturation should read 1.37 from state")


# --- get_color_grading_settings tests ---

func test_get_color_grading_settings_returns_color_grading_keys() -> void:
	var state := {
		"display": {
			"color_grading_filter_mode": 1,
			"color_grading_exposure": 0.5,
			"other_setting": true,
		}
	}
	var result := U_COLOR_GRADING_SELECTORS.get_color_grading_settings(state)
	assert_true(result.has("color_grading_filter_mode"),
		"Result should include color_grading_filter_mode")
	assert_true(result.has("color_grading_exposure"),
...
func test_get_color_grading_settings_excludes_non_cinema_keys() -> void:
	...
	assert_false(result.has("other_setting"),
		"Result should not include non-color_grading_ keys")
...
func test_selectors_handle_missing_display_slice() -> void:
	...
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_filter_intensity(state), 1.0, 0.001,
		"Missing display slice should return default filter_intensity=1.0")
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_contrast(state), 1.0, 0.001,
...
func test_selectors_handle_empty_state() -> void:
	...
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_contrast(state), 1.0, 0.001)
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_saturation(state), 1.0, 0.001)
	assert_almost_eq(U_COLOR_GRADING_SELECTORS.get_filter_intensity(state), 1.0, 0.001)
