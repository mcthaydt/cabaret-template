...
func test_registers_parent_with_manager() -> void:
	...
	assert_true(parent in ui_roots, "U_LocalizationRoot should register its parent with the localization manager")
...
func test_unregisters_parent_on_exit_tree() -> void:
	...
	var ui_roots: Array = _manager.get("_ui_roots")
	assert_true(parent in ui_roots, "Should be registered before removal")
	...
	ui_roots = _manager.get("_ui_roots")
	assert_false(parent in ui_roots, "U_LocalizationRoot should unregister parent on _exit_tree")
