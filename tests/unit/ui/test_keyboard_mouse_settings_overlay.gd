...
func test_apply_dispatches_keyboard_look_actions() -> void:
	...
	assert_true(_store.dispatched_actions.size() >= 2)
	assert_eq(_store.dispatched_actions[0].get("type"), U_InputActions.ACTION_SET_KEYBOARD_LOOK_ENABLED)
	assert_eq(_store.dispatched_actions[1].get("type"), U_InputActions.ACTION_SET_KEYBOARD_LOOK_SPEED)

	var enabled_payload: Dictionary = _store.dispatched_actions[0].get("payload", {})
	assert_true(bool(enabled_payload.get("enabled", false)))
	var speed_payload: Dictionary = _store.dispatched_actions[1].get("payload", {})
