...
const S_GAME_RULE_MANAGER := preload("res://scripts/ecs/systems/s_game_event_system.gd")
...
func test_checkpoint_area_as_sibling_connects_once() -> void:
	...
	# Phase 7D: System no longer tracks connections (uses event bus)
	# Verify that multiple emissions don't cause duplicate activations
	var history := U_ECS_EVENT_BUS.get_event_history()
	var activation_count: int = 0
	for ev in history:
		if ev.get("name") == StringName("checkpoint_activated"):
			var payload: Dictionary = ev.get("payload", {})
			if payload.get("spawn_point_id", StringName("")) == StringName("sp_mid"):
				activation_count += 1
	assert_eq(activation_count, 1, "Should only activate checkpoint once despite multiple body_entered signals")
