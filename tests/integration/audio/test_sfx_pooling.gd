...
##
## Validates pooled 3D SFX playback via M_SFXSpawner inside M_AudioManager.
...
func _first_player() -> AudioStreamPlayer3D:
	if M_SFX_SPAWNER._pool.is_empty():
		return null
	return M_SFX_SPAWNER._pool[0] as AudioStreamPlayer3D
...
func test_audio_manager_initializes_sfx_pool_container() -> void:
	assert_not_null(M_SFX_SPAWNER._container)
	assert_true(M_SFX_SPAWNER._container is Node3D)
	assert_eq(M_SFX_SPAWNER._container.name, "SFXPool")
	assert_eq(M_SFX_SPAWNER._container.get_parent(), _audio_manager)


func test_pool_size_is_16_players() -> void:
	assert_eq(M_SFX_SPAWNER._pool.size(), 16)
	assert_eq(M_SFX_SPAWNER._container.get_child_count(), 16)
...
func test_spawn_returns_null_for_empty_config() -> void:
	var player := M_SFX_SPAWNER.spawn_3d({})
	assert_null(player)


func test_spawn_returns_null_when_audio_stream_missing() -> void:
	var player := M_SFX_SPAWNER.spawn_3d({"position": Vector3.ZERO})
	assert_null(player)


func test_spawn_returns_null_when_audio_stream_wrong_type() -> void:
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": 123})
	assert_null(player)
...
func test_spawn_sets_position() -> void:
	...
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "position": position})
	assert_eq(player.global_position, position)


func test_spawn_sets_volume_db() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "volume_db": -3.0})
	assert_almost_eq(player.volume_db, -3.0, 0.001)


func test_spawn_sets_pitch_scale() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "pitch_scale": 1.25})
	assert_almost_eq(player.pitch_scale, 1.25, 0.001)


func test_spawn_sets_custom_bus() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "bus": "Footsteps"})
...
func test_finished_player_is_reused_by_next_spawn() -> void:
	var stream := AudioStreamWAV.new()
	var first := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream})
	first.emit_signal("finished")

	var second := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream})
...
func test_pool_exhaustion_returns_null_and_warns() -> void:
	...
	var exhausted := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream})
...
func test_can_route_sfx_to_ui_bus() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "bus": "UI"})
...
func test_spatial_audio_disabled_disables_attenuation_and_panning() -> void:
	M_SFX_SPAWNER.set_spatial_audio_enabled(false)
...
func test_spatial_audio_reenabled_restores_attenuation_and_panning() -> void:
	M_SFX_SPAWNER.set_spatial_audio_enabled(false)
	M_SFX_SPAWNER.set_spatial_audio_enabled(true)
...
func test_spawn_position_defaults_to_zero_when_wrong_type() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "position": "not a Vector3"})
	assert_eq(player.global_position, Vector3.ZERO)


func test_spawn_volume_defaults_to_zero_when_wrong_type() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "volume_db": "not a float"})
	assert_almost_eq(player.volume_db, 0.0, 0.001)


func test_spawn_pitch_defaults_to_one_when_wrong_type() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "pitch_scale": "not a float"})
	assert_almost_eq(player.pitch_scale, 1.0, 0.001)


func test_spawn_bus_casts_to_string() -> void:
	var stream := AudioStreamWAV.new()
	var player := M_SFX_SPAWNER.spawn_3d({"audio_stream": stream, "bus": StringName("SFX")})
	assert_eq(player.bus, "SFX")
