...
func tick(context: Dictionary, state_bag: Dictionary) -> Status:
	...
	for index in range(start_index, children.size()):
		...
		if child == null:
			push_error("RS_BTSelector.tick: child at index %d is null" % index)
			_clear_local_state(state_bag)
			return Status.FAILURE
		...
		match child_status:
			...
			Status.FAILURE:
				continue
			_:
				push_error("RS_BTSelector.tick: child returned invalid status %s" % str(child_status))
				_clear_local_state(state_bag)
				return Status.FAILURE
