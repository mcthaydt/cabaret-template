...
func _build_context() -> Dictionary:
	_binder.resolve(state_store, self, _on_action_dispatched)
	var rule_context: RefCounted = RS_RULE_CONTEXT.new()
	if _store != null:
...
func _evaluate_active_objectives(context: Dictionary) -> void:
	...
	var payload_variant: Variant = context.get(RS_RULE_CONTEXT.KEY_EVENT_PAYLOAD, {})
	if payload_variant is Dictionary:
	...
	for _iteration in range(max_iterations):
		...
		if not event_payload.is_empty():
			evaluation_context[RS_RULE_CONTEXT.KEY_EVENT_PAYLOAD] = event_payload.duplicate(true)
		var state: Dictionary = _store.get_state()
