class_name U_TemplateBaseScene2_5dBuilder
extends RefCounted

const FLOOR_SIZE := 5.0
...
func create_root() -> U_TemplateBaseScene2_5dBuilder:
	var node := Node3D.new()
...
func build_scene_objects() -> U_TemplateBaseScene2_5dBuilder:
	...
	var half := FLOOR_SIZE / 2.0
	var wall_y := WALL_HEIGHT / 2.0
	_add_wall(group, "SO_Wall_West", Vector3(-half, wall_y, 0), Vector3(THIN, WALL_HEIGHT, FLOOR_SIZE), &"wall_west", Vector3(-1, 0, 0))
	_add_wall(group, "SO_Wall_East", Vector3(half, wall_y, 0), Vector3(THIN, WALL_HEIGHT, FLOOR_SIZE), &"wall_east", Vector3(1, 0, 0))
	_add_wall(group, "SO_Wall_North", Vector3(0, wall_y, -half), Vector3(FLOOR_SIZE, WALL_HEIGHT, THIN), &"wall_north")
	_add_wall(group, "SO_Wall_South", Vector3(0, wall_y, half), Vector3(FLOOR_SIZE, WALL_HEIGHT, THIN), &"wall_south", Vector3(0, 0, 1))
	return self

func build_environment() -> U_TemplateBaseScene2_5dBuilder:
	var group := Node.new()
...
func build_systems() -> U_TemplateBaseScene2_5dBuilder:
	var group := Node.new()
	group.name = "Systems"
...
func build_managers() -> U_TemplateBaseScene2_5dBuilder:
	var group := Node.new()
	group.name = "Managers"
...
func build_entities() -> U_TemplateBaseScene2_5dBuilder:
	var group := Node.new()
	group.name = "Entities"
	group.set_script(MARKER_ENTITIES)
	_root.add_child(group)

	var player_scene: PackedScene = load(PLAYER_2_5D_PATH) as PackedScene
	if player_scene != null:
		var player := player_scene.instantiate()
		player.name = "E_Player"
		player.set_scene_file_path(PLAYER_2_5D_PATH)
		group.add_child(player)
	else:
		push_error("U_TemplateBaseScene2_5dBuilder: %s not found. Run build_prefab_player_2_5d first." % PLAYER_2_5D_PATH)
...
func save(path: String) -> bool:
	if _root == null:
		push_error("U_TemplateBaseScene2_5dBuilder: save() called before create_root()")
		return false
	_set_owner_recursive(_root, _root)
	var packed := PackedScene.new()
	var pack_result := packed.pack(_root)
	if pack_result != OK:
		push_error("U_TemplateBaseScene2_5dBuilder: pack() failed with code %d" % pack_result)
		return false
	var save_result := ResourceSaver.save(packed, path)
	if save_result != OK:
		push_error("U_TemplateBaseScene2_5dBuilder: ResourceSaver.save() failed with code %d" % save_result)
		return false
	return true

func build() -> Node3D:
	if _root == null:
		push_error("U_TemplateBaseScene2_5dBuilder: build() called before create_root()")
		return null
	return _root
...
func _add_floor(parent: Node3D) -> void:
	...
	box.size = Vector3(FLOOR_SIZE, THIN, FLOOR_SIZE)
