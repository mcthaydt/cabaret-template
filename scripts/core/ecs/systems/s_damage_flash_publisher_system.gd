...
const EVN_DAMAGE_FLASH_REQUEST := preload("res://scripts/core/events/ecs/evn_damage_flash_request.gd")
...
func _on_health_changed(event_data: Dictionary) -> void:
	...
	var event := EVN_DAMAGE_FLASH_REQUEST.new(entity_id, 1.0, StringName("damage"))
...
func _on_death(event_data: Dictionary) -> void:
	...
	var event := EVN_DAMAGE_FLASH_REQUEST.new(entity_id, 1.0, StringName("death"))
	U_ECS_EVENT_BUS.publish_typed(event)
