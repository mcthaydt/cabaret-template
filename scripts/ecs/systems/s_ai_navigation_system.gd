	...
	for entity_query_variant in entities:
		...
		if delta_xz.length() <= TARGET_REACHED_EPSILON:
			...
			_debug_log_navigation(
				entity_id,
				"move_vector=ZERO reason=target reached target=%s current=%s"
				% [str(target_position), str(current_position)],
				entity_query,
				movement_component
			)
			continue
		...
		_debug_log_navigation(
			entity_id,
			"move_vector=%s target=%s current=%s has_camera_basis=%s"
			% [str(move_vector), str(target_position), str(current_position), str(has_camera_basis)],
			entity_query,
			movement_component
		)
